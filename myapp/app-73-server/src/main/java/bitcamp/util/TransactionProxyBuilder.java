...
public class TransactionProxyBuilder {
  ...
  TransactionTemplate txTemplete;

  public TransactionProxyBuilder(PlatformTransactionManager txManager) {
    this.txTemplete = new TransactionTemplate(txManager);
  }
    ...
    return Proxy.newProxyInstance(this.getClass().getClassLoader(), new Class<?>[]{clazz},
            new InvocationHandler() {
              ...
              public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
                // Proxy 객체의 메서드를 호출할 때
                // @Transactional 이 붙은 메서드를 호출할 때는 TransactionTemplete 으로 처리하고
                // @Transactional 이 안붙은 메서드는 그냥 오리지널 객체의 메서드를 그대로 호출한다.

                // 1) Proxy 객체이 메서드와 일치하는 오리지널 작업 객체의 메서드를 가져온다.
                ...
                if (transactional != null) {
                  // @Transactional 애노테이션이 붙은 메서드라면 TransactionTemplete을 통해 실행하고
                  return txTemplete.execute(status -> {
                    System.out.printf("%s() - Transaction 경유!\n", originalMethod.getName());
