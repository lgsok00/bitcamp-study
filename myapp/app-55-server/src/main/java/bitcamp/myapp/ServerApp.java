...
public class ServerApp {
  ...
  private NettyOutbound processRequest(HttpServerRequest request, HttpServerResponse response) {
    ...
    try {
      // cookie 에서 Session ID 추출
      // Client Session ID 알아내기
      String sessionId = null;
      boolean firstVisit = false;

      // Client가 보낸 Cookie들 중에서 Session ID가 있는지 확인한다.
      List<Cookie> cookies = request2.allCookies().get(MYAPP_SESSION_ID);
      if (cookies != null) {
        // Session ID가 있으면 이 값을 가지고 Client를 구분한다.
        sessionId = cookies.get(0).value();
      } else {
        // Session ID가 없으면 이 Client를 구분하기 위해 새 Session ID를 발급한다.
        sessionId = UUID.randomUUID().toString();
        firstVisit = true;
      }

      // Session ID로 Client에게 배정된 HttpSession 객체를 찾는다.
      HttpSession session = sessionMap.get(sessionId);
      if (session == null) {
        // 현재 Client가 사용할 HttpSession 객체가 배정되지 않았다면, 새로 만든다.
        session = new HttpSession(sessionId);

        // 새로 만든 Session 객체를 Session ID를 사용하여 sessionMap에 보관한다.
        sessionMap.put(sessionId, session);
      }

      // Servlet에서 HttpSession 보관소를 사용할 수 있도록 HttpServletRequest에 담아 둔다.
      request2.setSession(session);

      if (firstVisit) {
        // Session ID가 없는 Client를 위해 새로 발급한 Session ID를 Cookie로 보낸다.
