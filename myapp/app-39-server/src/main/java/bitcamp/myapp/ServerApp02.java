...
public class ServerApp02 {
  ...
  // 클라이언트 요청을 처리할 DAO 객체를 Map 에 보관한다.
  ...
  public void execute() throws Exception {
    ...
    while (true) {
      ...
      if (dao == null) {
        // 만약 데이터를 처리할 DAO 를 찾지 못한다면 오류 정보를 클라이언트에게 보낸다.
        out.writeUTF(
            new ResponseEntity().status(ResponseEntity.ERROR).result("데이터를 찾을 수 없습니다!").toJson());
        continue;
      }

      // DAO 에 해당 메서드가 있는지 알아낸다.
      Method[] methods = dao.getClass().getDeclaredMethods(); // DAO 객체의 클래스 정보
      Method method = null; // 메서드 저장 레퍼런스
      ...
      switch (command) {
        ...
        // break;
        // case "reading/findBy":
        // Board reading = readingDao.findBy(request.getObject(Integer.class));
        // if (reading == null) {
        // response.status(ResponseEntity.SUCCESS);
        // } else {
        // response.status(ResponseEntity.SUCCESS).result(reading);
        // }
        // break;
        // case "reading/update":
