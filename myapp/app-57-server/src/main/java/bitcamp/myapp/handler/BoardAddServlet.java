...
public class BoardAddServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      // multipart의 각 part 데이터를 저장할 객체를 만드는 공장
      DiskFileItemFactory factory = new DiskFileItemFactory();

      // multipart 형식으로 넘어온 요청 파라미터를 분석하여 처리하는 객체
      ServletFileUpload upload = new ServletFileUpload(factory);

      // multipart 요청 파라미터를 분석
      ...
      // Web Application 환경정보를 알고 있는 객체 꺼내기
      ServletContext 웹애플리케이션환경정보 = request.getServletContext();

      // Web Application 환경정보에서 /upload/board 디렉토리의 실제 경로를 계산하여 추출한다.
      ...
      for (FileItem part : parts) {
        ...
        if (part.isFormField()) {
          ...
          // 파일 데이터
          // upload file은 배포 폴더에 저장한다.
          // 1) file을 저장할 때 사용할 이름을 준비한다.
          String filename = UUID.randomUUID().toString();

          // 2) upload 배포 폴더에 file을 저장한다.
          part.write(new File(uploadDir, filename));

          // 3) file 이름을 객체에 보관하여 목록에 추가한다.
