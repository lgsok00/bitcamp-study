...
// - 스레드 별로 DB Connection을 관리하고 제공하는 역할
// - 풀링(Pooling) 기법을 이용하여 DB Connection 재활용
// - GoF의 Flyweight 패턴
//
public class DataSource {
  ...
  public Connection getConnection() throws Exception {
    ...
    // Connection 꺼내기
    if (con == null) {
      // 현재 스레드에 장착된 DB Connection 객체가 없다면,
      if (connectionPool.size() > 0) {
        ...
        System.out.printf("[%s] - 새 DB Connection 생성!\n", Thread.currentThread().getName());
      }

      // connectionBox를 통해 새로 생성한 Connection 객체를 현재 스레드에 저장한다.
      connectionBox.set(con);
      System.out.printf("[%s] - 스레드에 Connection 객체 보관!\n", Thread.currentThread().getName());
    } else {
      System.out.printf("[%s] - 스레드에 보관된 Connection 사용!\n", Thread.currentThread().getName());
    }
    return con;
  }
  ...
  public void clean() {
    ...
    Connection con = connectionBox.get();
    if (con != null) {
      // 스레드가 사용한 DB Connection을 다른 스레드에서 사용할 수 있도록 커넥션 풀에 저장한다.
      // 다음에 다시 사용해야 하기 때문에 close() 하면 안된다.
      connectionPool.add(con);
      System.out.printf("[%s] - 커넥션 풀에 DB Connection 저장!\n", Thread.currentThread().getName());

      // 스레드에서는 제거한다.
      connectionBox.remove();
      System.out.printf("[%s] - 스레드에서 Connection 제거!\n", Thread.currentThread().getName());
    }
  }
}
